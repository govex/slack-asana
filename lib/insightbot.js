...
InsightBot.prototype._replyWithRandomJoke = function (originalMessage) {
  ...
  if(originalMessage.text.toLowerCase() == '*cancel') {
    ...
    self._talkToSlack('group', "All righty then...operation canceled.")
    
  }
  
  else if(listeningForMore) {
    
    self._taskWorkflow(originalMessage);
    
  }
  
  else if(authed && !listeningForMore && !listeningForStatus) {
    
    self._talkToSlack('group', "Standby - phoning home.");
    self._callInsightly(originalMessage);
    
  } else if(!authed) {
    
    self._talkToSlack('group',  "I'm sorry. Authentication to Insightly has failed, and I can't phone home. Give me a sec and try again.")
    self.authenticate();
  ...
  request(
    ...
    function (error, response, body) {
       ...
       if(error) {
         ...
         self._talkToSlack('group', "I'm sorry. Authentication to Insightly has failed, and I can't phone home. Ping @phiden and let her know.")
         authed = false;
         ...
         self._talkToSlack('group', "You're connected to Insightly's API! Right now, you can ask @insightly to: `show <person>'s tasks`, `show my tasks`, `show events`, `create a task`, and `show users`.");
...
InsightBot.prototype._callAPI = function(command, verb) {
  ...
  if(authed) {
    ...
    //this loop is standing on its own
    if(command != 'Users' && command != 'User') {
      ...
      request(options, function(error, response, data) {
        ...
        if (error) throw new Error(error);
          ...
          self._executeOnData(json);
        }
      
      /*request(options, function (error, response, data) {
      
        var json = self._parseJSON(data);
        console.log("testing new function: ", json);
      ...
      //this 'else' is what gets called if you ask to show users.
      console.log("what is posting?");
...
InsightBot.prototype._executeOnData = function(json) {
...
InsightBot.prototype._parseCommand = function(msg) {
  ...
  if(verb == undefined || command == undefined ) {
    
    console.log("something's bad. throw an error.");
    self._talkToSlack('group', "Sorry, I didn't understand what you asked me for. Please try again!");
    self._resetSlackListeners();
...
//instead of changing the function in a switch based on verb, have an if/else set in the command function itself for get/post
...
InsightBot.prototype._loadInsightlyUsers = function() {
  ...
  var options = { method: 'GET',
    ...
    request(options, function (error, response, data) {
      ...
      var json = self._parseJSON(data);
      console.log("testing new function 2: ", json);
...
InsightBot.prototype._parseJSON = function(data){
  
  console.log('parsing json');
  var unparsed = JSON.stringify(eval("(" + data + ")"));
  return JSON.parse(unparsed);
  
}

InsightBot.prototype._talkToSlack = function(medium, message) {
  
  if(medium == 'group') {
    
    self.postMessageToGroup('slackly-testing', message, {as_user:true});
    
  } else {
    
    self.postMessageToChannel('slackly-testing', message, {as_user:true});
  }
...
InsightBot.prototype._getTasksData = function() {
  ...
  if(status == undefined || status == '') {
    ...
    if(status == undefined && listeningForStatus == false) {
      ...
      self._talkToSlack('group', 'Did you want `completed`, `in progress`, or `not started` tasks?');
    }
...
InsightBot.prototype._enterTaskCreation = function(msg, continuance) {
  ...
  if(continuance == '') {
    ...
    self._talkToSlack('group', "Ok. Let's create a task. Who should we assign it to? Type `*cancel` to cancel.")
  ...
  if(continuance == 'checkUser'){
    ...
    request(options, function (error, response, data) {
      ...
      var json = self._parseJSON(data);
      console.log("testing new function 3: ", json);
      ...
      for(var key in json) {
        ...
        if(msg.toLowerCase() == json[key].FIRST_NAME.toLowerCase() || msg.toLowerCase() == json[key].LAST_NAME.toLowerCase() || msg.toLowerCase() == json[key].FIRST_NAME + ' ' + json[key].LAST_NAME) {
          ...
          self._talkToSlack('group', "You're creating a task for " + json[key].FIRST_NAME + ". Enter a title, please.")
          userChecked = true;
    ...
    titleAdded = true;
    self._talkToSlack('group', "Ok, the title's been added. Please add a description or some details:");
    ...
    detailsAdded = true;
    self._talkToSlack('group',  "Ok, the details have been added to the task, and I'm going to create it now.");
    ...
    request(options, function (error, response, data) {
      ...
      if(data != '') {
        ...
        var json = self._parseJSON(data);
        console.log("testing new function 4: ", json);
       
        //you have task ID here -- make this message more useful.
        self._talkToSlack('group', "Your task has been added! You can edit it directly in Insightly: " + "https://govex.insight.ly/Tasks/TaskDetails/" + json.TASK_ID);
        self._resetSlackListeners();
      
      } else {
        
        self._talkToSlack('group', "Rats. Something went wrong. Please try again.")
        self._resetSlackListeners();
      }
    })
...
InsightBot.prototype._taskWorkflow = function(originalMessage) {
  
  self._talkToSlack('group', "I'm listening for more. Type `*cancel` to cancel. " + verb + ' ' + command);
