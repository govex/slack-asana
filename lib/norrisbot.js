  ...
  request(
    ...
    function (error, response, body) {
       ...
       if(error) {
         ...
         self.postMessageToGroup('slackly-testing', "You're connected to Insightly's API! Right now, you can `show <person>'s tasks`, `show events`, `create a task`, and `show users`.", {as_user: true});
...
NorrisBot.prototype._getMyUser = function() {
  
  console.log('finding my user: ', messageObject);
...
NorrisBot.prototype._postTasksToSlack = function(data) {
  ...
  if(hasUser && user != 'my') {
    ...
    tasks = "*These are all " + myUser + "'s open Insightly tasks:*\n";
    for(var key in data) {
