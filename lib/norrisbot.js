...
NorrisBot.prototype._callInsightly = function(message) {
  ...
  var commandRaw = this._getInsightlyCommand(msg);
  
  console.log('command is: ', command, verb);
  
  if(command == 'nope' || command == 'undefined') {
    
    this.postMessageToGroup('slackly-testing', "So sorry; you've asked for something I can't do yet. Try again?", {as_user: true});
    //eventually create /bot for a glossary of commands here
  } else {
    
    this._callAPI(command, verb, callback);
...
var _postUsersToSlack = function(data) {
  ...
  console.log('what is self' , NorrisBot);
  NorrisBot.postMessageToGroup('slackly-testing', name, {as_user: true});
}


var _postTasksToSlack = function(data) {
  
  data = JSON.parse(data);
  console.log(typeof(data));
  
  var name = '*These are all your Insightly tasks:*\n';
  
  for(var key in data) {
    
    name += data[key].TITLE + ' with a status of ' + data[key].STATUS + 'is due on ' + data[key].DUE_DATE + '\n';
    //console.log(name);
